...
#Create table of FIPS x Year to check missingness and drop counties if missing all data for a cause of death
##Substance Use Disorder
```{r}
tb <- JailMortalityData %>% select(FIPS, Year, SubUseDisDeaths)
tb_wide <- spread(tb, Year, SubUseDisDeaths)
tb_wide$na_count <- apply(tb_wide, 1, function(x) sum(is.na(x)))
table(tb_wide$na_count)
#drop counties from analysis if missing all data for Substance Use Disorder deaths
dropfips <- tb_wide %>% filter(na_count == 31 | na_count == 30) %>% select(FIPS)
JailMortalityData <- JailMortalityData %>% filter(!FIPS %in% dropfips$FIPS)
length(unique(JailMortalityData$FIPS))
```

##Infectious Disease
```{r}
tb <- JailMortalityData %>% select(FIPS, Year, InfectDeaths)
tb_wide <- spread(tb, Year, InfectDeaths)
tb_wide$na_count <- apply(tb_wide, 1, function(x) sum(is.na(x)))
table(tb_wide$na_count)
#drop counties from analysis if missing all data for Infectious deaths
dropfips <- tb_wide %>% filter(na_count == 31 | na_count == 30) %>% select(FIPS)
JailMortalityData <- JailMortalityData %>% filter(!FIPS %in% dropfips$FIPS)
length(unique(JailMortalityData$FIPS))
```
...
##Suicide
```{r}
tb <- JailMortalityData %>% select(FIPS, Year, SuicDeaths)
tb_wide <- spread(tb, Year, SuicDeaths)
tb_wide$na_count <- apply(tb_wide, 1, function(x) sum(is.na(x)))
table(tb_wide$na_count)
#drop counties from analysis if missing all data for Suicide deaths
dropfips <- tb_wide %>% filter(na_count == 31| na_count == 30) %>% select(FIPS)
JailMortalityData <- JailMortalityData %>% filter(!FIPS %in% dropfips$FIPS)
length(unique(JailMortalityData$FIPS))
```
...
##Diabetes
```{r}
tb <- JailMortalityData %>% select(FIPS, Year, DiabDeaths)
tb_wide <- spread(tb, Year, DiabDeaths)
tb_wide$na_count <- apply(tb_wide, 1, function(x) sum(is.na(x)))
table(tb_wide$na_count)
#drop counties from analysis if missing all data for Diabetes deaths
dropfips <- tb_wide %>% filter(na_count == 31| na_count == 30) %>% select(FIPS)
JailMortalityData <- JailMortalityData %>% filter(!FIPS %in% dropfips$FIPS)
length(unique(JailMortalityData$FIPS))
```
...
##Cerebro
```{r}
tb <- JailMortalityData %>% select(FIPS, Year, CerebroDeaths)
tb_wide <- spread(tb, Year, CerebroDeaths)
tb_wide$na_count <- apply(tb_wide, 1, function(x) sum(is.na(x)))
table(tb_wide$na_count)
#drop counties from analysis if missing all data for Cerebro deaths
dropfips <- tb_wide %>% filter(na_count == 31| na_count == 30) %>% select(FIPS)
JailMortalityData <- JailMortalityData %>% filter(!FIPS %in% dropfips$FIPS)
length(unique(JailMortalityData$FIPS))
```
...
##Chronic Lower resp
```{r}
tb <- JailMortalityData %>% select(FIPS, Year, LwrRspDeaths)
tb_wide <- spread(tb, Year, LwrRspDeaths)
tb_wide$na_count <- apply(tb_wide, 1, function(x) sum(is.na(x)))
table(tb_wide$na_count)
#drop counties from analysis if missing all data for Chronic lower resp deaths
dropfips <- tb_wide %>% filter(na_count == 31| na_count == 30) %>% select(FIPS)
JailMortalityData <- JailMortalityData %>% filter(!FIPS %in% dropfips$FIPS)
length(unique(JailMortalityData$FIPS))
```
...
##Injury
```{r}
tb <- JailMortalityData %>% select(FIPS, Year, UnintInjDeaths)
tb_wide <- spread(tb, Year, UnintInjDeaths)
tb_wide$na_count <- apply(tb_wide, 1, function(x) sum(is.na(x)))
table(tb_wide$na_count)
#drop counties from analysis if missing all data for Injury deaths
dropfips <- tb_wide %>% filter(na_count == 31| na_count == 30) %>% select(FIPS)
JailMortalityData <- JailMortalityData %>% filter(!FIPS %in% dropfips$FIPS)
length(unique(JailMortalityData$FIPS))
```
...
##Heart disease
```{r}
tb <- JailMortalityData %>% select(FIPS, Year, HeartDeaths)
tb_wide <- spread(tb, Year, HeartDeaths)
tb_wide$na_count <- apply(tb_wide, 1, function(x) sum(is.na(x)))
table(tb_wide$na_count)
#drop counties from analysis if missing all data for Heart disease deaths
dropfips <- tb_wide %>% filter(na_count == 31| na_count == 30) %>% select(FIPS)
JailMortalityData <- JailMortalityData %>% filter(!FIPS %in% dropfips$FIPS)
length(unique(JailMortalityData$FIPS))
```
...
##Cancer
```{r}
tb <- JailMortalityData %>% select(FIPS, Year, CancerDeaths)
tb_wide <- spread(tb, Year, CancerDeaths)
tb_wide$na_count <- apply(tb_wide, 1, function(x) sum(is.na(x)))
table(tb_wide$na_count)
#drop counties from analysis if missing all data for Cancer deaths
dropfips <- tb_wide %>% filter(na_count == 31| na_count == 30) %>% select(FIPS)
JailMortalityData <- JailMortalityData %>% filter(!FIPS %in% dropfips$FIPS)
length(unique(JailMortalityData$FIPS))
```
...
```{r}
AnalysisData <- AnalysisData %>% filter(!is.na(County_ADPRate) & !is.na(Deaths))
nrow(AnalysisData)
length(unique(AnalysisData$FIPS))
```
...
pdf(file = "/Users/sk4538/Sync/Mass Incarceration and Public Health/County Jail and Health/Cause of Death/Figures/Final/Figure1.pdf", width = 9, height = 6)
plot1 <- ggplot(data = Fig1, aes(x=Year, y=County_ADPRate)) +
  geom_boxplot() +
  theme_tufte() +
  ylab("Jail incarceration rate (per 1,000)") +
  scale_x_discrete(breaks = pretty(as.numeric(rev(Fig1$Year)), n = 15)) +
...
plot2 <- ggplot(data = forplot, aes(x=Year, y=ChangeinVal)) +
  ...
  xlab(" ") +
  scale_x_discrete(breaks = pretty(as.numeric(rev(forplot$Year)), n = 15)) +
  coord_cartesian(clip = 'off') +
  ggtitle("B")+
  theme(
    text = element_text(size=14),
...
#Plot mortality data
```{r}
Fig2 <- AnalysisData  %>% select(c(Year, Population, CancerDeaths:InfectDeaths)) %>% group_by(Year) %>% summarize_all(sum, na.rm=TRUE) 
Fig2$Year <- as.numeric(Fig2$Year) + 1
Fig2$Year <- as.factor(Fig2$Year)
Fig2_long <- gather(Fig2, cause, deaths, CancerDeaths:InfectDeaths, factor_key=TRUE)
Fig2_long$rate <- Fig2_long$deaths/Fig2_long$Population*1000
Fig2_long$label <- ifelse(Fig2_long$cause == "CancerDeaths", "Malignant neoplasm", ifelse(Fig2_long$cause == "CerebroDeaths", "Cerebrovascular disease", ifelse(Fig2_long$cause == "LwrRspDeaths", "Chronic lower respiratory disease", ifelse(Fig2_long$cause == "DiabDeaths", "Diabetes", ifelse(Fig2_long$cause == "SubUseDisDeaths", "Substance-induced", ifelse(Fig2_long$cause == "HeartDeaths", "Heart disease", ifelse(Fig2_long$cause == "SuicDeaths", "Suicide", ifelse(Fig2_long$cause == "UnintInjDeaths", "Unintentional injury", ifelse(Fig2_long$cause == "InfectDeaths","Infectious disease", NA)))))))))
```
...
library(ggthemes)
library(directlabels)
pdf(file = "/Users/sk4538/Sync/Mass Incarceration and Public Health/County Jail and Health/Cause of Death/Figures/Final/Figure2.pdf", width = 11, height = 4)
Fig2_long <- Fig2_long %>% filter(Year %in% c(1988:2018))
ggplot(data = Fig2_long, aes(x=Year, y=rate, color=cause, group=cause)) +
  geom_line() +
...
pdf(file = "/Users/sk4538/Sync/Mass Incarceration and Public Health/County Jail and Health/Cause of Death/Figures/Final/Figure3.pdf", width = 6, height = 4)
...
#Replacing Suppressed Values - Sensitivity Analysis
##Replace all missing mortality counts with 0
```{r}
SensSuppr0 <- AnalysisData
SensSuppr0[c("Deaths", "CancerDeaths", "CerebroDeaths","LwrRspDeaths","DiabDeaths","SubUseDisDeaths","HeartDeaths","SuicDeaths","UnintInjDeaths","InfectDeaths")][is.na(SensSuppr0[c("Deaths", "CancerDeaths", "CerebroDeaths","LwrRspDeaths","DiabDeaths","SubUseDisDeaths","HeartDeaths","SuicDeaths","UnintInjDeaths","InfectDeaths")])] <- 0
```

```{r}
#One-year lag
## Substance Use
Substance1 <- glm(SubUseDisDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr0, family=quasipoisson)

## Suicide
Suic1 <- glm(SuicDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr0, family=quasipoisson)

## Cancer
Cancer1 <- glm(CancerDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr0, family=quasipoisson)

## Cerebrovascular
Cerebro1 <- glm(CerebroDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr0, family=quasipoisson)

## Lower Respiratory
LwrRsp1 <- glm(LwrRspDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr0, family=quasipoisson)

## Diabetes
Diab1 <- glm(DiabDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr0, family=quasipoisson)

## Heart Disease
Heart1 <- glm(HeartDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr0, family=quasipoisson)

## Unintentional Injury
UnintInj1 <- glm(UnintInjDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr0, family=quasipoisson)

## Infectious disease
Infect1 <- glm(InfectDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr0, family=quasipoisson)
```

##Replace all missing mortality counts with 9
```{r}
SensSuppr9 <- AnalysisData
SensSuppr9[c("Deaths", "CancerDeaths", "CerebroDeaths","LwrRspDeaths","DiabDeaths","SubUseDisDeaths","HeartDeaths","SuicDeaths","UnintInjDeaths","InfectDeaths")][is.na(SensSuppr9[c("Deaths", "CancerDeaths", "CerebroDeaths","LwrRspDeaths","DiabDeaths","SubUseDisDeaths","HeartDeaths","SuicDeaths","UnintInjDeaths","InfectDeaths")])] <- 9
```

```{r}
#One-year lag
## Substance Use
Substance1 <- glm(SubUseDisDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr9, family=quasipoisson)

## Suicide
Suic1 <- glm(SuicDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr9, family=quasipoisson)

## Cancer
Cancer1 <- glm(CancerDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr9, family=quasipoisson)

## Cerebrovascular
Cerebro1 <- glm(CerebroDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr9, family=quasipoisson)

## Lower Respiratory
LwrRsp1 <- glm(LwrRspDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr9, family=quasipoisson)

## Diabetes
Diab1 <- glm(DiabDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr9, family=quasipoisson)

## Heart Disease
Heart1 <- glm(HeartDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr9, family=quasipoisson)

## Unintentional Injury
UnintInj1 <- glm(UnintInjDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr9, family=quasipoisson)

## Infectious disease
Infect1 <- glm(InfectDeaths ~ ADPper1000 + PovertyRate + TotalCrime_rate + Percent_Black_county  + MedianAge + Unemployment_Rate + Party + CUSTRATE + Year + FIPS + offset(log(Population)), data=SensSuppr9, family=quasipoisson)
```
