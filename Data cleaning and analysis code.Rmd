...
JailAnalysisData$White_ConfinedRate <- (JailAnalysisData$white_jail_pop/JailAnalysisData$white_pop_15to64)*100
JailAnalysisData$Black_ConfinedRate <-(JailAnalysisData$black_jail_pop/JailAnalysisData$black_pop_15to64)*100
```
...
#Create a list of all fips and all years in jaildata
```{r}
...
```{r}
JailMortalityData$Region <- ifelse(JailMortalityData$state %in% c("CT", "MA", "ME", "NH", "RI", "VT"), 1, ifelse(JailMortalityData$state %in% c("NJ", "NY", "PA"), 2, ifelse(JailMortalityData$state %in% c("IN", "IL", "MI", "OH", "WI"), 3, ifelse(JailMortalityData$state %in% c("IA", "KS", "MN", "MO", "NE", "ND", "SD"), 4, ifelse(JailMortalityData$state %in% c("DE", "DC", "FL", "GA", "MD", "NC", "SC", "VA", "WV"), 5, ifelse(JailMortalityData$state %in% c("AL", "KY", "MS", "TN"), 6, ifelse(JailMortalityData$state %in% c("AR", "LA", "OK", "TX"), 7, ifelse(JailMortalityData$state %in% c("AZ", "CO", "ID", "NM", "MT", "UT", "NV", "WY"), 8, ifelse(JailMortalityData$state %in% c("AK", "CA", "HI", "OR", "WA"), 9, NA)))))))))
...
```{r}
test <- AnalysisData  %>% select(c(Year, total_pop, total_jail_pop)) %>% group_by(Year) %>% summarize_all(sum, na.rm=TRUE) 
test$jailrate <- test$total_jail_pop/test$total_pop*100000
```

```{r}
library(ggthemes)
library(directlabels)
png(file = "/Users/sk4538/Sync/Mass Incarceration and Public Health/County Jail and Health/Cause of Death/Figures/Figure 2_plot jail data.png", res = 600, width = 9, height = 4, units = "in")

ggplot(data = subset(test, Year %in% c(1987:2017)), aes(x=Year, y=jailrate, group=1)) +
  geom_line() +
  theme_tufte() +
  ylab("Jail Incarceration Rate (per 100,000)") +
...
test_long$rate <- test_long$deaths/test_long$Population*100000
...
# CONTINUOUS
```{r}
#Unit of exposure: 0.1 percentage point increase, change ADP to per 1000
...
ratesub <- predict(Substance1, newdata=datasub, type="response")

pred_ratesub <- cbind(datasub, ratesub)

plot_pred_ratesub1 <- pred_ratesub %>% 
  group_by(ADPper1000) %>% 
  summarise("Predicted Rate" = mean(ratesub, na.rm=TRUE)) %>% 
...
ratesub <- predict(Substance5, newdata=datasub, type="response")
...
ratesub <- predict(Substance10, newdata=datasub, type="response")
...
ratesui <- predict(Suic1, newdata=datasui, type="response")

pred_ratesui <- cbind(datasui, ratesui)

plot_pred_ratesui1 <- pred_ratesui %>% 
  group_by(ADPper1000) %>% 
  summarise("Predicted Rate" = mean(ratesui, na.rm=TRUE)) %>% 
...
ratesui <- predict(Suic5, newdata=datasui, type="response")
...
ratesui <- predict(Suic10, newdata=datasui, type="response")
...
ratelwr <- predict(LwrRsp1, newdata=datalwr, type="response")

pred_ratelwr <- cbind(datalwr, ratelwr)

plot_pred_ratelwr1 <- pred_ratelwr %>% 
  group_by(ADPper1000) %>% 
  summarise("Predicted Rate" = mean(ratelwr, na.rm=TRUE)) %>% 
...
ratelwr <- predict(LwrRsp5, newdata=datalwr, type="response")
...
ratelwr <- predict(LwrRsp10, newdata=datalwr, type="response")
...
rateinf <- predict(Infect1, newdata=datainf, type="response")

pred_rateinf <- cbind(datainf, rateinf)

plot_pred_rateinf1 <- pred_rateinf %>% 
  group_by(ADPper1000) %>% 
  summarise("Predicted Rate" = mean(rateinf, na.rm=TRUE)) %>% 
...
rateinf <- predict(Infect5, newdata=datainf, type="response")
...
rateinf <- predict(Infect10, newdata=datainf, type="response")
...
png(file = "/Users/sk4538/Sync/Mass Incarceration and Public Health/County Jail and Health/Cause of Death/Figures/PredictedCurvCOD_7-30-20.png", res = 600, width = 6, height = 4, units = "in")

ggplot(plotdata, aes(x=ADPper1000, y=`Predicted Rate`, group=Lag)) +
  ...
  ylab("Predicted Mortality Rate (per 100,000)\n") 
dev.off()
```
...
#Sensitivity analysis with same outcome years for everyone: 1998-2018
##One-year lag
```{r}
Sensitivity1 <- AnalysisData %>% filter(Year > "1995")
Sensitivity5 <- AnalysisData %>% filter(Year > "1991" & Year < "2014")
